    ...
    if(deployed_way == 1){
        if(from_open){
            ...
            for (auto& purchase_server : purchase_servers) {
                ...
                (purchase_server->A_vm_id.size() + purchase_server->B_vm_id.size()+purchase_server->AB_vm_id.size() !=0)) {
...
void SolveProblem(Statistics& statistic) {
    Cmp cmp;
...
int main(int argc, char* argv[]) {
    ...
    statistic.compute_statistics_of_VM(sold_vms);
    //statistic.del_bad_server(sold_servers);
    statistic.compute_statistics_of_server(sold_servers);
    SolveProblem(statistic);
#ifdef PRINTINFO
    _end = clock();
#endif
#ifdef PRINTINFO
