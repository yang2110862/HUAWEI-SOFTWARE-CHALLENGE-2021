...
int GiveMyOffers(vector<RequestData>& intraday_requests) {
    // 备份模拟部署前的服务器状态。
    int server_remainders_copy[purchase_servers.size()][4];
    int server_num = purchase_servers.size();
    for (int i = 0; i < server_num; ++i) {
        server_remainders_copy[i][0] = purchase_servers[i]->A_remain_cpu;
        server_remainders_copy[i][1] = purchase_servers[i]->A_remain_memory;
        server_remainders_copy[i][2] = purchase_servers[i]->B_remain_cpu;
        server_remainders_copy[i][3] = purchase_servers[i]->B_remain_memory;
    }
    ...
    // 恢复服务器状态。
    for (int i = 0; i < server_num; ++i) {
        purchase_servers[i]->A_remain_cpu = server_remainders_copy[i][0];
        purchase_servers[i]->A_remain_memory = server_remainders_copy[i][1];
        purchase_servers[i]->B_remain_cpu = server_remainders_copy[i][2];
        purchase_servers[i]->B_remain_memory = server_remainders_copy[i][3];
    }
